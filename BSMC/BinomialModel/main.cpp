...
double calculateDiscountFactor(double const interestRate, double const maturity) const
{
    return exp(-interestRate * maturity);    
}

double calculateDrift(double const interestRate, double const volatility) const 
{
    return interestRate - 0.5 * pow(volatility, 2);
}


double calculateDt(double const maturity, double const numberOfSteps) const
{
    return maturity / numberOfSteps;
}

double calculateChangeOfAsset(
        double const volatility, double const dt, double const drift) const
{
    return sqrt(pow(volatility, 2)  * dt + pow(drift * dt, 2));
}

double calculateUpProbability(double const drift, double const dt, double const changeOfAsset) const
{
    return 0.5 + 0.5 * drift * dt / changeOfAsset;
}

double calculateDownProbability(double const upProbability) const
{
